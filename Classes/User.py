...
E_transmit_max = Z_max*Ptrans_max*10/B  # J
...
class User:
    ...
    def get_E_transmit(self, datarate):
        return (Z[self.num]*self.current_ptrans/(datarate*datarate_max))/E_transmit_max
    ...
    def get_energy_ratio(self, server):
        ...
        E_transmit = self.get_E_transmit(self.get_datarate(server)*datarate_max)*E_transmit_max
        return E_local/E_transmit
    ...
    def E_transmit_ext(self):   # E_transmit with externality calculation
        E_transmit_ext_list = [Z[self.num]*self.current_ptrans/(dr*datarate_max) for dr in self.datarate_ext_list]
